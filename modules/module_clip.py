...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)   #相当于把二维图片映射到一维，长度为width
...
class CLIP(nn.Module):
    ...
    def get_config(pretrained_clip_name="ViT-B/16"):
        model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ViT-B-16.pt")
