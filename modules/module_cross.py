...
class CrossAttn(nn.Module):
    ...
    def scores(self,x, y):
        x_m = torch.sqrt(torch.sum(x * x, dim=-1))
        y_m = torch.sqrt(torch.sum(y * y))
        return torch.matmul(x, y.transpose(-1, -2)).squeeze(-1) / (x_m * y_m)

    def forward(self, visual, sequence):
        visual_out = torch.zeros(visual.shape[0],1,visual.shape[-1])
        batch = min(visual.shape[0], sequence.shape[0])
        scores = self.scores(visual[:batch], sequence[:batch])
        most_sim = torch.argmax(scores, dim=1)
        visual_out[:batch] = visual[range(batch), most_sim].unsqueeze(1)
        return visual_out.cuda() 
